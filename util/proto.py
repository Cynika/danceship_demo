...
class Proto:
    ...
    def unpack(self, buf):
        ...
        if self.ver == 0:
            # 这里做回调
            return catch(self.body.decode('utf-8'))
        elif self.ver == 2:
            # 解压
        ...
        if 'cmd' in bar:
            match bar['cmd']:
                case 'LIVE_OPEN_PLATFORM_DM':
                    data = bar['data']
                    return ({'timeline': data['timestamp'], 'uid': data['uid'], 'nickname': data['uname'],
                             'text': data['msg'], 'uface': data['uface'], 'state': 0})
                case 'LIVE_OPEN_PLATFORM_SEND_GIFT':
                    data = bar['data']
                    return {'timeline': data['timestamp'], 'uid': data['uid'], 'nickname': data['uname'],
                            'text': f"礼物 {data['gift_name']} {data['gift_num']} {data['price']}",
                            'uface': data['uface'], 'state': 0}
